...
CREATE OR REPLACE VIEW CRM_PROJECT.STG.STG_OBJECTIVES AS
SELECT
  COUNTRY                             AS country,
  MAX                                 AS max_target,
  MIN                                 AS min_target,
  ...
  CONCAT(YEARQUARTER, '-', COUNTRY)   AS year_quarter_country
FROM CRM_PROJECT.PUBLIC.RAW_OBJECTIVES
WHERE COUNTRY IS NOT NULL AND YEARQUARTER IS NOT NULL;